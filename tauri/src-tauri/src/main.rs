...
//! The Cycle desktop shell — a window showing the bundled site, and nothing
//! else.
//!
...
//! here needed no change to the app at all. Everything Cycle does — the
...
use cycle_shell::config::APP_SCHEME;
use cycle_shell::oauth::{AWAIT_PATH, BEGIN_PATH};
use cycle_shell::webroot::webroot_exists;
...
fn main() {
    tauri::Builder::default()
        ...
        // `cycle_shell::webroot` for the three properties this arrangement is
        ...
        // `cycle_shell::oauth`), which can be minutes, so it answers from a
        ...
        .setup(|app| {
            ...
            if cycle_shell::config::remote_app_url().is_none() && !webroot_exists(&root) {
                let message = format!(
                    "Cycle could not find the app it is meant to show.\n\n\
                     Looked in: {}\n\n\
        ...
        .expect("the Cycle desktop shell could not start");
}
