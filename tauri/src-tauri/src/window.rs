...
//! Every decision about that file is `cycle_shell::window_state`'s; this
//! file only asks the window and the monitors.

use cycle_shell::config::{
    app_origin, is_internal_url, remote_app_url, start_url, BRAND_BG, WINDOW_TITLE,
};
use cycle_shell::window_state::{
...
const DEFAULT_WINDOW: WindowState = cycle_shell::window_state::DEFAULT_STATE;
