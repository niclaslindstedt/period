...
use cycle_shell::config::{app_origin, is_internal_url, start_url, APP_HOST, APP_SCHEME};
...
fn each_platform_gets_the_origin_it_grants() {
    assert_eq!(app_origin(false), "cycle://localhost");
    assert_eq!(app_origin(true), "http://cycle.localhost");
}
...
fn the_window_opens_on_the_entry_page() {
    assert_eq!(
        start_url("cycle://localhost"),
        "cycle://localhost/index.html"
    );
...
fn our_own_pages_navigate_in_the_window() {
    let origin = "cycle://localhost";
    assert!(is_internal_url("cycle://localhost", origin, None));
    assert!(is_internal_url(
        "cycle://localhost/index.html",
        origin,
        None
    ));
    assert!(is_internal_url("cycle://localhost/privacy/", origin, None));
}
...
fn anything_else_does_not() {
    let origin = "cycle://localhost";
    for outside in [
        "https://github.com/niclaslindstedt/period",
        "https://cycle.niclaslindstedt.se/",
        "file:///etc/passwd",
        // The near-miss that a naive `starts_with` on the scheme would admit.
        "cycle://localhost.example.com/",
    ] {
        assert!(
...
/// A launch pointed at a remote build (`CYCLE_APP_URL`) navigates within THAT
/// site too — otherwise every in-app link would bounce to the browser.
#[test]
fn a_remote_launch_navigates_within_its_own_site() {
    let origin = "cycle://localhost";
    let remote = Some("https://cycle.niclaslindstedt.se/preview/");
    assert!(is_internal_url(
        "https://cycle.niclaslindstedt.se/preview/index.html",
        origin,
        remote
    ));
