...
# Cycle — the desktop app

A desktop wrapper around Cycle for **Windows, macOS and Linux**. It is a thin
...
minimum. Everything Cycle does — the document, the settings, the sync engine —
...
| `src-tauri/src/protocol.rs` | Answering `cycle://` off the bundled `webroot/`               |
...
`cargo test -p cycle-shell` runs the entire decision layer on a machine with a
...
| Variable        | Effect                                                               |
| --------------- | -------------------------------------------------------------------- |
| `CYCLE_APP_URL` | Load a remote URL instead of the bundled site (the `/preview/` slot) |
| `CYCLE_WEBROOT` | Serve the site from somewhere else without rebuilding                |
...
identifier (`dev.local.cycle`). What an installed copy is called and the
