...
export const RULES: StoreRules = {
  ...
  brand: {
    ...
    // behaviour does.
    privacyUrl: "https://apps.agilator.se/cycle/privacy/",
    supportUrl: "https://apps.agilator.se/cycle/support/",
  },

  // Cycle ships on the App Store alone. There is no Mac App Store record and
  ...
  apple: {
    ...
    advisory: {
      ...
      // keeps the reader's own daily reports and draws them back as a
      // calendar and charts. Two rows ARE judgement calls, and both are
      // answered on the side of saying more rather than less — an answer
      // that understates what the app shows is a rejection, one that
      // overstates it costs a slightly higher age rating and nothing else.
      violenceCartoonOrFantasy: "NONE",
      violenceRealistic: "NONE",
      violenceRealisticProlongedGraphicOrSadistic: "NONE",
      horrorOrFearThemes: "NONE",
      profanityOrCrudeHumor: "NONE",
      // Menstruation is not a mature theme; it is the subject of the app.
      // But a day's report has a yes/no for sex and one for lust — the
      // ovulatory signals the forecast reads — and the app says those words
      // back in its calendar and charts. That is health data, not suggestive
      // content, but it is a plain mention of sexual activity, which is what
      // this row asks about at its mildest level.
      matureOrSuggestiveThemes: "INFREQUENT_OR_MILD",
      sexualContentOrNudity: "NONE",
      sexualContentGraphicAndNudity: "NONE",
      alcoholTobaccoOrDrugUseOrReferences: "NONE",
      // The question asks what the APP provides, and this app does provide
      // something: from the reader's own reports it predicts the next period,
      // estimates ovulation, and labels days "Fertile" and "Not fertile". That
      // is not a diagnosis, a dosage or a treatment — but a "not fertile"
      // label is exactly the kind of statement a reader could act on as
      // health guidance, so it is declared, at the mild level. The forecast
      // itself says it is "an estimate from your own reports — not medical
      // advice, and not contraception", and the privacy page that the listing
      // links to says it is not a medical device.
    ...
    contact: {
      ...
      // Nothing to sign in to and no permission prompt. A reviewer opening a
      // fresh install sees an empty calendar until a day is reported, so the
      // review notes say to report a period day and watch the forecast fill
      // in — no demo account is involved.
      demoRequired: false,
    },
